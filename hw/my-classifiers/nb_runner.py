...
print("Artif noise:\t",run(a_train, a_train, 'target', NaiveBayes(), one_hot=False))

a_train, a_test = load_synth_sep()
print("Artif sep:\t", run(a_train, a_test, 'target', NaiveBayes(), one_hot=False))

a_train, a_test = load_adult(False, 50)
print("Adult:\t", run(a_train, a_test, 'target', NaiveBayes(), one_hot=False))